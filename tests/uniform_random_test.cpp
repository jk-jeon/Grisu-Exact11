...
void uniform_random_test(std::size_t number_of_tests, TypenameString&& type_name_string)
{
	using extended_significand_type =
		typename jkj::grisu_exact11_detail::common_info<Float>::extended_significand_type;
	...
	for (std::size_t test_idx = 0; test_idx < number_of_tests; ++test_idx) {
		...
		if (std::is_same<Float, float>::value) {
			...
			d2s_buffered(double(x), buffer2);
		}

		std::string view1(buffer1);
		std::string view2(buffer2);
