...
public:
	...
	>;
	void run(std::size_t number_of_iterations, std::string const& float_name, output_type& out)
	{
		assert(number_of_iterations >= 1);
		char buffer[40];

		for (auto const& name_func_pair : name_func_pairs_) {
			typename output_type::iterator result_array_itr;
			bool is_inserted;
			std::tie(result_array_itr, is_inserted) = out.insert_or_assign(
	...
	output_type run(std::size_t number_of_iterations, std::string const& float_name)
	{
		output_type out;
	...
	void register_function(std::string const& name, void(*func)(Float, char*))
	{
