...
#ifndef JKJ_GRISU_EXACT11
#define JKJ_GRISU_EXACT11
...
// [[maybe_unused]]
	...
	#define JKJ_MAYBE_UNUSED [[maybe_unused]]
#else
	#if defined(_MSC_VER)
		#if _MSC_VER >= 1911 && _MSVC_LANG >= 201703L
			#define JKJ_MAYBE_UNUSED [[maybe_unused]]
		#else
			#define JKJ_MAYBE_UNUSED
		#endif
	#elif defined(__GNUC__)
		#define JKJ_MAYBE_UNUSED __attribute__((unused))
	#else
		#define JKJ_MAYBE_UNUSED
	#endif
#endif
...
namespace jkj {
	namespace grisu_exact11_detail {
		////////////////////////////////////////////////////////////////////////////////////////
			...
			JKJ_IF_CONSTEXPR (std::is_same<UInt, std::uint32_t>::value) {
				JKJ_IF_CONSTEXPR(sizeof(unsigned long) == 4) {
					...
					static_assert(sizeof(unsigned long) == 4 || sizeof(unsigned int) == 4,
						"jkj::grisu_exact11: at least one of unsigned int and unsigned long should be 4 bytes long");
					index = __builtin_ctz((unsigned int)x);
				}
			}
			else {
				static_assert(sizeof(unsigned long long) == 8,
					"jkj::grisu_exact11: unsigned long long should be 8 bytes long");
		...
		template<>
		struct divisibility_test_table_holder<std::uint32_t> {
			static constexpr std::size_t size = 12;
			static constexpr divisibility_test_table_entry<std::uint32_t> table[12] = {
				{ 4294967295u, 1u },
				{ 858993459u, 3435973837u },
				{ 171798691u, 3264175145u },
				{ 34359738u, 652835029u },
				{ 6871947u, 989560465u },
				{ 1374389u, 197912093u },
				{ 274877u, 1757569337u },
				{ 54975u, 2928494245u },
		...
		template<>
		struct divisibility_test_table_holder<std::uint64_t> {
			static constexpr std::size_t size = 24;
			static constexpr divisibility_test_table_entry<std::uint64_t> table[24] = {
				{ 18446744073709551615ull, 1ull },
				{ 3689348814741910323ull, 14757395258967641293ull },
				{ 737869762948382064ull, 10330176681277348905ull },
				{ 147573952589676412ull, 2066035336255469781ull },
				{ 29514790517935282ull, 15170602326218735249ull },
				{ 5902958103587056ull, 6723469279985657373ull },
				{ 1180591620717411ull, 8723391485480952121ull },
				{ 236118324143482ull, 16502073556063831717ull },
				{ 47223664828696ull, 14368461155438497313ull },
				{ 9444732965739ull, 10252389860571520109ull },
				{ 1888946593147ull, 5739826786856214345ull },
				{ 377789318629ull, 1147965357371242869ull },
				{ 75557863725ull, 3918941886216158897ull },
				{ 15111572745ull, 11851834821468962749ull },
				{ 3022314549ull, 6059715779035702873ull },
				{ 604462909ull, 8590640785290961221ull },
				{ 120892581ull, 16475523416025833537ull },
				{ 24178516ull, 14363151127430897677ull },
				{ 4835703ull, 13940676669711910505ull },
				{ 967140ull, 2788135333942382101ull },
				{ 193428ull, 15315022325756117713ull },
				{ 38685ull, 10441702094635044189ull },
				{ 7737ull, 5777689233668919161ull },
				{ 1547ull, 15912933105701425125ull }
			};
		};

		template <class UInt>
		constexpr bool divisible_by_power_of_5(UInt x, unsigned int exp) noexcept {
			assert(exp < divisibility_test_table_holder<UInt>::size);
			auto const& entry = divisibility_test_table_holder<UInt>::table[exp] ;
		...
		struct common_info {
			...
			using extended_significand_type = typename std::conditional<
				sizeof(Float) == 4,
				std::uint32_t,
				std::uint64_t>::type;
			...
			using cache_entry_type = typename std::conditional<
				sizeof(Float) == 4,
				std::uint64_t,
				uint128>::type;
	...
	struct fp_t<Float, false> {
		using extended_significand_type =
			typename grisu_exact11_detail::common_info<Float>::extended_significand_type;
	...
	{
	private:
		using common_info = grisu_exact11_detail::common_info<Float>;
	...
	namespace grisu_exact11_correct_rounding {
		enum tag_t {
			do_not_care_tag,
			tie_to_even_tag,
			tie_to_odd_tag,
		...
		struct do_not_care {
			...
			{
				return grisu_exact11_detail::grisu_exact11_impl<Float>::template compute<return_sign,
					typename std::remove_cv<typename std::remove_reference<IntervalTypeProvider>::type>::type, do_not_care>(br);
			}
		};
		...
		struct tie_to_even {
			...
			{
				return grisu_exact11_detail::grisu_exact11_impl<Float>::template compute<return_sign,
					typename std::remove_cv<typename std::remove_reference<IntervalTypeProvider>::type>::type, tie_to_even>(br);
			}
		};
		...
		struct tie_to_odd {
			...
			{
				return grisu_exact11_detail::grisu_exact11_impl<Float>::template compute<return_sign,
					typename std::remove_cv<typename std::remove_reference<IntervalTypeProvider>::type>::type, tie_to_odd>(br);
			}
		};
		...
		struct tie_to_up {
			...
			{
				return grisu_exact11_detail::grisu_exact11_impl<Float>::template compute<return_sign,
					typename std::remove_cv<typename std::remove_reference<IntervalTypeProvider>::type>::type, tie_to_up>(br);
			}
		};
		...
		struct tie_to_down {
			...
			{
				return grisu_exact11_detail::grisu_exact11_impl<Float>::template compute<return_sign,
					typename std::remove_cv<typename std::remove_reference<IntervalTypeProvider>::type>::type, tie_to_down>(br);
			}
		};
	}

	// Dispatch to the appropriate operator() overloading of grisu_exact11_impl<Float>
	namespace grisu_exact11_rounding_modes {
		enum tag_t {
			to_nearest_tag,
	...
	namespace grisu_exact11_detail {
		...
		template <class Float>
		struct grisu_exact11_impl : private common_info<Float>
		{
			using extended_significand_type =
			...
			struct power_of_10 {
				static constexpr extended_significand_type value = compute_power(extended_significand_type(10), e);
			};

			template <bool has_sign, class = void>
			struct handle_sign {
				void operator()(bit_representation_t<Float> br, fp_t<Float, has_sign>& ret_value) {
					ret_value.is_negative = br.is_negative();
				}
			};
			template <class dummy>
			struct handle_sign<false, dummy> {
				void operator()(bit_representation_t<Float>, fp_t<Float, false>&) {
				}
			};

			template <class Float_t = Float, class = void>
			struct handle_fr_overflow;

			template <class dummy>
			struct handle_fr_overflow<float, dummy> {
				template <class CacheEntryType>
				common_info<float>::extended_significand_type
					operator()(CacheEntryType const& cache, int minus_beta) noexcept {
					return common_info<float>::extended_significand_type(cache >>
						extended_precision >> minus_beta);
				}
			};

			template <class dummy>
			struct handle_fr_overflow<double, dummy> {
				template <class CacheEntryType>
				common_info<double>::extended_significand_type
					operator()(CacheEntryType const& cache, int minus_beta) noexcept {
					return cache.high() >> minus_beta;
				}
			};
			...
			static fp_t<Float, return_sign> compute(bit_representation_t<Float> br) noexcept
			{
				...
				handle_sign<return_sign>{}(br, ret_value);
				...
				auto const cache = jkj::grisu_exact11_detail::get_cache<Float>(-minus_k);
					...
					grisu_exact11_rounding_modes::left_closed_directed_tag)
				{
					...
					if (fr == 0) {
						zi = handle_fr_overflow<>{}(cache, minus_beta);
					}
					else {
				...
				ret_value.significand = zi / power_of_10<initial_kappa>::value;
				auto r = zi % power_of_10<initial_kappa>::value;
				...
				// the search will be performed only once. Hence, we do not need to do that.
				{
					...
					auto const quotient = std::uint32_t(r) / std::uint32_t(power_of_10<initial_kappa - lambda>::value);
					auto const new_r = std::uint32_t(r) % std::uint32_t(power_of_10<initial_kappa - lambda>::value);
					...
					divisor = power_of_10<initial_kappa - 2>::value;
					...
					JKJ_IF_CONSTEXPR (CorrectRoundingSearch::tag !=
						grisu_exact11_correct_rounding::do_not_care_tag &&
				...
				decrease_kappa_by_1_label:
					...
					divisor = power_of_10<initial_kappa - 1>::value;
					goto boundary_adjustment_label;
				}

			increasing_search_label:
				// Perform binary search
				divisor = power_of_10<initial_kappa>::value;
				...
				JKJ_IF_CONSTEXPR (sizeof(Float) == 4) {
					...
					// This procedure strictly depends on our specific choice of these parameters:
					static_assert(sizeof(Float) != 8 || max_kappa - initial_kappa < 16,
						"jkj::grisu_exact11: something is wrong!");
			...
			boundary_adjustment_label:
				...
				if (!interval_type.include_right_endpoint() && r == 0) {
					constexpr integer_check_case_id case_id =
						IntervalTypeProvider::tag == grisu_exact11_rounding_modes::to_nearest_tag ?
			...
			correct_rounding_search_label:
					...
					grisu_exact11_rounding_modes::to_nearest_tag)
				{
					...
					auto epsiloni = compute_delta<grisu_exact11_rounding_modes::left_closed_directed_tag>(
					...
					if (displacement <= epsiloni) {
						...
						if (epsiloni == 0) {
							...
							if (yi == approx_y) {
								JKJ_IF_CONSTEXPR (CorrectRoundingSearch::tag ==
									grisu_exact11_correct_rounding::tie_to_even_tag ||
									CorrectRoundingSearch::tag ==
									grisu_exact11_correct_rounding::tie_to_odd_tag)
								{
									...
									grisu_exact11_correct_rounding::tie_to_up_tag)
								{
									--steps;
								}
								else {
			...
			template <class dummy>
			struct compute_mul_impl<float, dummy> {
				template <class ExtendedSignificandType, class CacheEntryType>
				std::uint32_t operator()(ExtendedSignificandType f, CacheEntryType const& cache) noexcept {
					return umul96_upper32(f, cache);
				}
				template <class CacheEntryType>
				std::uint64_t operator()(CacheEntryType const& cache) noexcept {
					return cache;
				}
			};

			template <class dummy>
			struct compute_mul_impl<double, dummy> {
				template <class ExtendedSignificandType, class CacheEntryType>
				std::uint64_t operator()(ExtendedSignificandType f, CacheEntryType const& cache) noexcept {
					return umul192_upper64(f, cache);
				}
				template <class CacheEntryType>
				std::uint64_t operator()(CacheEntryType const& cache) noexcept {
					return cache.high();
			...
			{
				return compute_mul_impl<>{}(f, cache) >> minus_beta;
			}

			template <grisu_exact11_rounding_modes::tag_t tag>
			static std::uint32_t compute_delta(JKJ_MAYBE_UNUSED bool is_edge_case,
				cache_entry_type const& cache, int minus_beta) noexcept
			{
				...
				using intermediate_type = typename std::conditional<sizeof(Float) == 4,
					cache_entry_type, extended_significand_type>::type;

				intermediate_type r = compute_mul_impl<>{}(cache);

				// For nearest rounding
				JKJ_IF_CONSTEXPR (tag == grisu_exact11_rounding_modes::to_nearest_tag)
				{
					if (is_edge_case) {
						r = (r >> 1) + (r >> 2);
					}
				...
				else JKJ_IF_CONSTEXPR (tag == grisu_exact11_rounding_modes::left_closed_directed_tag)
				{
					return std::uint32_t(r >> (intermediate_precision - q_mp_m1 + minus_beta));
				}
			...
			static bool is_product_integer(JKJ_MAYBE_UNUSED extended_significand_type f,
				int exponent, JKJ_MAYBE_UNUSED int minus_k) noexcept
			{
			...
			template <grisu_exact11_rounding_modes::tag_t tag>
			static bool equal_fractional_parts(JKJ_MAYBE_UNUSED extended_significand_type fl,
				JKJ_MAYBE_UNUSED int exponent, JKJ_MAYBE_UNUSED int minus_k) noexcept
			{
				JKJ_IF_CONSTEXPR (tag == grisu_exact11_rounding_modes::to_nearest_tag)
				{
					// Generic case
			...
			template <grisu_exact11_rounding_modes::tag_t tag, class IntervalType>
			...
			template <extended_significand_type lambda,
				grisu_exact11_rounding_modes::tag_t tag,
			...
			{
				auto const quotient = ret_value.significand / power_of_10<lambda>::value;
				auto const new_r = r + divisor * (ret_value.significand % power_of_10<lambda>::value);
				...
				r = new_r;
				divisor *= power_of_10<lambda>::value;
				return true;
			}
		};
	}
	...
	namespace grisu_exact11_case_handlers {
		struct assert_finite {
			template <class Float>
			void operator()(JKJ_MAYBE_UNUSED bit_representation_t<Float> br) const
			{
				assert(br.is_finite());
			}
		};
	...
	template <bool return_sign = true, class Float,
		...
		class CaseHandler = grisu_exact11_case_handlers::assert_finite
	>
	fp_t<Float, return_sign> grisu_exact11(Float x,
